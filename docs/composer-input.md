...
- 实时过滤：输入非空后列表**只显示**名称匹配的条目（精确 > 前缀 > 包含 >
  子序列，fzf 风格；`../` 恒显示以便回退上级），清空查询恢复全部；无匹配时仍显示
  空框提示而不是让菜单消失
- 候选框跟随输入（follow）：过滤后当前目录没有匹配时，会向子目录树（最多 3 层，跳过
  `.git` / `node_modules` / `target` 等重型目录）找最接近的条目并跳转选中——输入
  `@abc` 而根目录没有 `abc*` 时，浏览器直接跳到 [docs/abc_ref.md] 所在目录并选中它；
  继续输入会停留在该目录，修改/回退段则回到 workspace 根重新匹配
...
| 点击输入区右上角外框上的 `⛶` | 悬停高亮：点击将输入区在「放大（约 5/8 屏）」与「自动高度」之间切换；无快捷键，纯鼠标。按钮位于卡片外框（cap 行）右端，文字全满也不会遮挡 |
