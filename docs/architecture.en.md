...
Client process: independent Cordis root
  ...
  harness-view    injects tuiCommands + tuiOverlay + acpClient + acpSessionStatus;
                  ...
                  internal product forcedHarness is empty by default and, when
                  set, wins persisted defaultHarness at startup; it is not a CLI
                  startup argument; /harness only updates defaultHarness;
