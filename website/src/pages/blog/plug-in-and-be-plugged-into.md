---
layout: ../../components/ContentPage.astro
title: "《我为DeepSeek Harness补齐了这四个重要的基础Agent组件--DSH Plugin最佳实践》 | Martty"
description: "从 Plugin 的装载、组合与继承，走到 Web 双树、动态 Plugin、Preset，以及 Martty 的‘插与被插’。"
path: /blog/plug-in-and-be-plugged-into
eyebrow: 架构与实践
article: true
lang: zh-CN
publishedTime: "2026-08-23"
publishedLabel: 发布于 2026 年 8 月 23 日
showDescription: false
---

![DeepSeek Harness 四个 Agent 组件与 DSH Profile 的关系](/blog/dsh-plugin-best-practices/01-framework-cover.png)

## TL;DR

- Plugin 让 Agent 的新能力不必等进核心版本，也能先从社区长出来。
- DSH Plugin 有何不同：它不只**组合** Plugin，还允许 Agent 沿 Scope 父链**继承**能力。复用更方便，代价是开发模型更复杂，隐式依赖和同名遮蔽也更难排查。
...
- **[Martty](https://github.com/openma-ai/Martty)**：TUI插件，也是一个 ACP Client，可以连接 DSH 或其他 ACP Agent；同时开放主题、命令、面板和动态 Cordis Client Plugin 扩展点，既能被插入，也允许别人继续插入。  
  GitHub：<https://github.com/openma-ai/Martty>
- **[DeepSeek Harness ACP](https://github.com/openma-ai/deepseek-harness-acp)**：让 DSH 能够接入 Zed、TUI 等 ACP Client。  
  GitHub：<https://github.com/openma-ai/deepseek-harness-acp>
- **[DSH Agent Plugins Bridge](https://github.com/openma-ai/dsh-agents-plugins)**：把 Agent Plugins、Codex、Claude Code、Pi 插件转换成 DSH 能力。  
  GitHub：<https://github.com/openma-ai/dsh-agents-plugins>
- **[DSH MCP Apps](https://github.com/openma-ai/dsh-mcp-apps)**：在 DSH 已支持 MCP 的基础上补齐 MCP Apps，让 Tool 交付可交互界面。  
  GitHub：<https://github.com/openma-ai/dsh-mcp-apps>

回头看，这四项工作把同一套 Plugin 机制推到了四个不同的边界。[DeepSeek Harness ACP](https://github.com/openma-ai/deepseek-harness-acp) 把 DSH 的 Session、Tool 和权限翻译成 ACP 消息，让 Zed、Martty 等 Client 可以直接连接 DSH；[DSH Agent Plugins Bridge](https://github.com/openma-ai/dsh-agents-plugins) 把其他 Agent 生态的插件转换成 DSH 能管理的能力；[DSH MCP Apps](https://github.com/openma-ai/dsh-mcp-apps) 在现有 MCP 支持之上补齐应用层；[Martty](https://github.com/openma-ai/Martty) 则让TUI插件本身也成为一个可以继续扩展的组件。
...
[DeepSeek Harness ACP](https://github.com/openma-ai/deepseek-harness-acp) 的 stdio transport 几乎就是一个最小的 DSH Plugin：
...
![DSH Plugin 从 package 安装到 Fiber 运行及 dispose 回收的流程](/blog/dsh-plugin-best-practices/02-flowchart-plugin-loading.png)

[DeepSeek Harness ACP](https://github.com/openma-ai/deepseek-harness-acp) 的演进让这条分界逐渐清楚。最初它更像一个自带启动逻辑的独立适配器：找到 Harness、创建 Session、占用 stdio，然后把两边消息互译。独立运行没有问题，一旦希望它进入 Zed、Martty 或任意现有 profile，自行启动第二套 Host 就会复制凭证、Session、Tool 和持久化状态。
...
[Agent Plugins](https://agent-plugins.org/) 标准在这些生态之上提供了一套可移植的 package 规范：用 `plugin.json` 描述插件，以 `skills/` 和 `mcp.json` 承载标准能力。它主要解决“一个插件包如何被不同 Agent 识别”，DSH 继续回答“包里的能力进入运行时后怎样相互依赖、由谁持有”。
...
如果你还想享受到传统的 Harness 插件生态，[Agent Plugins Bridge](https://github.com/openma-ai/dsh-agents-plugins) 负责这层转换。它以 Agent Plugins 1.0 作为可移植核心规范，再为 Codex、Claude Code 和 Pi 增加兼容 adapter：Discovery 只读识别，用户明确 Import 后才复制到当前 Profile，Activation 再把各项能力变成 Loader row。
...
![双向 Web Plugin 在 Node Host 与浏览器中产生两个独立实例](/blog/dsh-plugin-best-practices/03-comparison-host-client.png)

[DSH MCP Apps](https://github.com/openma-ai/dsh-mcp-apps) 就是一个双向 Plugin。我们开始开发时所基于的 DSH 版本，已经通过 `dsh-mcp-client` 提供 MCP 连接、Tool 发现与执行等基础能力，还没有支持 MCP Apps。Apps 需要从 Tool definition 的 `_meta` 找到 `ui://` Resource，再读取界面内容、建立 AppBridge，并安全运行 Server 提供的 HTML。
...
![AgentPreset 的能力 Scope 与 PermissionPreset 的执行策略对比](/blog/dsh-plugin-best-practices/04-comparison-presets.png)
...
## 不仅要插，还要被插：[Martty](https://github.com/openma-ai/Martty) 的例证
...
![Martty 作为 ACP Client 插入 DSH，并允许第三方 Client Plugin 插入自己](/blog/dsh-plugin-best-practices/05-framework-martty.png)
